...
import {TrashIcon, PencilAltIcon, GlobeAltIcon} from "@heroicons/react/outline"
	...
	return (
		<main className="min-h-screen">
			<ul role="list" className="divide-y divide-gray-700">
				{posts.map((post) => (
					<li key={post.id} className="p-4 group hover:outline hover:outline-2 hover:outline-gray-100/25 rounded-lg hover:bg-gray-700">
								...
								<div className="flex items-center justify-between min-h-[5em]">
									...
									<div className="flex flex-col text-right justify-between">
										...
										<div className="flex flex-row gap-4">
											<button className="hidden gap-1 p-2 bg-gray-300 rounded-lg group-hover:flex text-sm">
												<GlobeAltIcon className="h-5 w-5" /> View
											</button>
											<button className="hidden gap-1 p-2 bg-gray-300 rounded-lg group-hover:flex text-sm">
												<PencilAltIcon className="h-5 w-5" /> Edit
											</button>
											<button className="hidden gap-1 p-2 bg-red-500 rounded-lg group-hover:flex text-sm">
